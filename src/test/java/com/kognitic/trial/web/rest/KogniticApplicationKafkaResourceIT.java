...
import com.kognitic.trial.config.KafkaProperties;
...
class KogniticApplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        KogniticApplicationKafkaResource kafkaResource = new KogniticApplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/kognitic-application-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kognitic-application-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
